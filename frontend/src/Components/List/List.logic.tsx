...
export const useLogic = () => {
  ...
  const columns: TableColumn<ListItemsProps>[] = [
    ...
    {
      name: 'Event',
      selector: (row) => row.event[0].toUpperCase() + row.event.slice(1),
      sortable: true,
      width: '100px',
    },
  ...
  function checkTtimeNow(id: number) {
    ...
    timeCheck = dateFormat.format(date);

    completeList.forEach((guest, index: number) => {
      if (guest.id === id) {
        completeList[index].time = timeCheck;
        if (guest.check === 'N/A' || guest.check === 'Check-Out') {
          completeList[index].check = 'Check-In';
        } else {
          completeList[index].check = 'Check-Out';
        }
      }
    });
